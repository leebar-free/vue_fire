<template>
    <v-container fluid>
        <v-card>
            <v-alert type="error" v-if="!this.$store.state.user.emailVerified" >이메일을 확인해 주세요.</v-alert>
            <v-card-text>
                <v-row>
                    <v-col cols="5">
...
export default {
    ...
    methods: {
        ...
        async changeName () {
            ...
            const user = this.$firebase.auth().currentUser
            const dispName = `${this.form.lastName} ${this.form.firstName}`
            console.log("changeName :", dispName)
